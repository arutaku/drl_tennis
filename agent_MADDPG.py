...
class Critic(nn.Module):
    ...
    def __init__(self, state_size, action_size, n_h1, n_h2, seed):
        ...
        super().__init__()
        self.seed = torch.manual_seed(seed)
        self.fcs1 = nn.Linear(state_size * 2, n_h1)
        self.bn1 = nn.BatchNorm1d(n_h1)
        self.fc2 = nn.Linear(n_h1 + action_size * 2, n_h2)
    ...
    def forward(self, state, action, state_other, action_other):
        x = F.relu(self.bn1(self.fcs1(torch.cat((state, state_other), dim=1))))
        x = torch.cat((x, action, action_other), dim=1)
        x = F.relu(self.fc2(x))
        return self.fc3(x)
...
class Agent(object):
    ...
    def step(self, states, actions, rewards, next_states, dones):
        ...
        # print('Saving: ', states, actions, rewards, next_states, dones)
    ...
    def learn(self, experiences, gamma):
        ...
        gamma: float
            Discount factor.
        """
        states, actions, rewards, next_states, dones, states_other, actions_other, next_states_other = experiences

        # Critic network
        # Next-state actions (from actor) and Q-values (critic) from targets.
        actions_next = self.actor_target(next_states)
        actions_next_other = self.actor_target(next_states_other)
        Q_targets_next = self.critic_target(next_states, actions_next, next_states_other, actions_next_other)   # HERE: next_states *, actions_next *
        # Q-values targets for current states (y_i).
        Q_targets = rewards + (gamma * Q_targets_next * (1 - dones))
        # Critic loss.
        Q_expected = self.critic_local(states, actions, states_other, actions_other)                            # HERE: states *, actions *
        ...
        actions_other_pred = self.actor_local(states_other)
        actor_loss = -self.critic_local(states, actions_pred, states_other, actions_other_pred).mean()          # HERE: states *, actions_pred *
        # Optimization.
        ...
        """
        states, actions, rewards, next_states, dones, states_other, actions_other = experiences
...
class ReplayBuffer(object):
    ...
    def __init__(self, buffer_size, batch_size):
        ...
        self.experience = namedtuple(
                'Experience',
                field_names=['state', 'action', 'reward', 'next_state', 'done', 'state_other', 'action_other', 'next_state_other'])
    
    def add(self, state, action, reward, next_state, done, state_other, action_other, next_state_other):
        """Add a new experience to the buffer."""
        e = self.experience(state, action, reward, next_state, done, state_other, action_other, next_state_other)
    ...
    def sample(self):
        ...
        states_other = torch.from_numpy(np.vstack([e.state_other for e in experiences if e is not None])).float().to(device)
        actions_other = torch.from_numpy(np.vstack([e.action_other for e in experiences if e is not None])).float().to(device)
        next_states_other = torch.from_numpy(np.vstack([e.next_state_other for e in experiences if e is not None])).float().to(device)
        return states, actions, rewards, next_states, dones, states_other, actions_other, next_states_other
